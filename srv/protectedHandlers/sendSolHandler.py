...
def handle_send_sol(uuid, data):
    try:
        ...
        if not from_private_key or not to_public_key or amount_sol is None:
            return jsonify(standard_response(False, "Missing required parameters"))

        # Ensure amount_sol is a float
        try:
            amount_sol = float(amount_sol)
        except ValueError:
            return jsonify(standard_response(False, "Invalid amount format"))
        ...
        try:
            ...
            return jsonify(standard_response(True, "Transaction successful", {"log" : transaction_log, "warning" : ""}))
        
        except Exception as e:
            return jsonify(standard_response(True, "Transaction successful", {"log" : transaction_log, "warning" : "Could not log txn"}))
